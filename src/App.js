...
function App() {
  ...
  useEffect(() => {
    const clockTask = setTimeout(() => {
      if (running === true && timeLeft > 0) setTimeLeft(timeLeft - 1);
      if (timeLeft === 1) {
        document.getElementById("beep").play();
        setSession(!session);
      }
      if (timeLeft === 0) {
        if (!session) setTimeLeft(sessionLength * 60);
        else setTimeLeft(breakLength * 60);
      }
    }, 1000);
    return () => {
      clearTimeout(clockTask);
    };
  }, [timeLeft, running, breakLength, session, sessionLength]);
  ...
  function handleClick(event) {
    ...
    switch (event.target.id) {
      ...
      case "session-decrement":
        ...
        if (running === false) setTimeLeft((sessionLength - 1) * 60);
        break;
  ...
  return (
    <div id="app">
      <div id="clock">
        <label id="break-label">Break Length: {breakLength}</label>
        <br />
        ...
        <br />
        <label id="session-label">Session Length: {sessionLength}</label>
        <br />
        ...
        <audio
          src="http://pomodoroclock.mikeshecket.com/beep.mp3"
          id="beep"
        ></audio>
      </div>
    </div>
